...
class SceneManager:
    ...
    DRAW_RACKETS_ACTION= DrawRacketsAction(VIDEO_SERVICE)
    ...
    def _add_volleys(self, cast):
        cast.clear_actors(VOLLEYS_GROUP)
        text = Text(VOLLEYS_FORMAT, FONT_FILE, FONT_SMALL, ALIGN_LEFT)
        position = Point(HUD_MARGIN, HUD_MARGIN)
        label = Label(text, position)
        cast.add_actor(VOLLEYS_GROUP, label)
    ...
    def _add_rackets(self, cast):
        ...
        velocity = Point(0, 0)
        body1 = Body(position, size, velocity)
        animation = Animation(RACKET_IMAGES, RACKET_RATE)
        racket1 = Racket(body1, animation)
        cast.add_actor(RACKET_GROUP, racket1)

        # For player 2
        x2 = CENTER_X - RACKET_WIDTH / 2
        y2 = FIELD_TOP + RACKET_HEIGHT
        position = Point(x2, y2)
        size = Point(RACKET_WIDTH, RACKET_HEIGHT)
        velocity = Point(0, 0)
        body2 = Body(position, size, velocity)
        animation = Animation(RACKET_IMAGES, RACKET_RATE)
        racket2 = Racket(body2, animation)
    ...
    def _add_output_script(self, script):
        ...
        script.add_action(OUTPUT, self.DRAW_RACKETS_ACTION)
